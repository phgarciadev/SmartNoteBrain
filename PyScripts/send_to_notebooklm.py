...
def send_to_notebooklm(file_path):
    ...
    # LOCK SYSTEM: Garante que apenas um script rode por vez (sequencial)
    import fcntl
    lock_file_path = "/tmp/notebooklm_automation.lock"
    # Garantir que o arquivo exista
    open(lock_file_path, 'a').close()
    lock_file = open(lock_file_path, "r+")
    
    print("⏳ Aguardando sua vez na fila de automação (lock)...")
    try:
        ...
        if not is_port_open(port):
             if not start_chrome(port, debug_dir):
                print("❌ Falha crítica: Não foi possível iniciar o Chrome.")
                sys.exit(1)
        ...
        print(f"❌ Erro no sistema de Lock ou Conexão: {e}")
    finally:
        # Avisos e liberação do lock
